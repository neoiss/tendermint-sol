...
library Encoder {
    ...
    /* Vanilla SimpleValidator struct requires PublicKey structure to represent a PK, and that's gas expensive.
     * To optimize gas usage the Validator.pub_key is of `bytes` type, so the below code is required to transparently
     * serialize object to match the CommitSig.
     */
    function _estimatePK(Validator.Data memory r) internal pure returns (uint256) {
        uint256 e;
    ...
    function _estimateNew(Validator.Data memory r) internal pure returns (uint256) {
        uint256 e;
